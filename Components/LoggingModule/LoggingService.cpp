...
void LoggingService::ProcessFlashDump()
{
	...
	while ((dumpSector < maxSector || (dumpSector == maxSector && dumpOffset < maxOffset)) && !doneDump) {
		...
		for (uint32_t i = 0; i + RECORD_SIZE <= CHUNK_SIZE; i += RECORD_SIZE) //each record is 20 bytes, Chunck size is 500 to prevent hardfault
		{
			...
			else if(type == LoggingData::THERMOCOUPLE){
			    int32_t  temp_centiC_;
			    int32_t  coldJunction_centiC_;
			    uint8_t  fault_;

				memcpy(&temp_centiC_, sectorBuf + i + 5,  sizeof(int32_t));
				memcpy(&coldJunction_centiC_, sectorBuf + i + 9,  sizeof(int32_t));
				memcpy(&fault_, sectorBuf + i + 13, sizeof(uint8_t));

				SOAR_PRINT("%s Timestamp=%lu temp=%ld coldJunction=%ld fault=%lu\n",
							SensorTypeName(type), timestamp,
							(long)temp_centiC_, (long)coldJunction_centiC_, (long)fault_);
